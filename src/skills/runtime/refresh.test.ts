...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "openclaw-refresh-dir-symlink-probe-"));
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      try {
        ...
        await fs.symlink(
          targetSkillDir,
          path.join(groupedLinkDir, "linked-skill"),
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      try {
        ...
        await fs.symlink(
          outsideDir,
          path.join(pluginDir, "skills", "untrusted"),
          directorySymlinkType,
        );
        const pluginSkills = await import("../loading/plugin-skills.js");
        vi.mocked(pluginSkills.resolvePluginSkillDirs).mockReturnValueOnce([pluginDir]);
